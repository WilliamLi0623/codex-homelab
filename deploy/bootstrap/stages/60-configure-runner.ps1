...
$r=$cfg["recovery_dir"]
foreach($f in @("node-v20.19.2-linux-x64.tar.xz","webcodex-runner-0.4.1","webcodex-0.4.1","openai-codex-0.155.0.tgz","openai-codex-0.155.0-linux-x64.tgz","codex-auth.json")){ Invoke-Proxmox "scp -q -o BatchMode=yes -o ConnectTimeout=10 -o StrictHostKeyChecking=yes $r/$f webcodex@${ip}:/tmp/$f" | Out-Null }
