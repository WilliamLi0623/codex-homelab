...
& scp.exe @runnerSshArgs $tmp "${target}:/tmp/runner.toml" | Out-Null
...
& scp.exe @runnerSshArgs $tmpUnit "${target}:/tmp/webcodex-runner.service" | Out-Null
Remove-Item -Force $tmpUnit
if($LASTEXITCODE -ne 0){throw "runner unit copy failed"}
& ssh.exe @runnerSshArgs $target "mkdir -p ~/.config/webcodex/v2; install -m 600 /tmp/runner.toml ~/.config/webcodex/v2/runner.toml; rm -f /tmp/runner.toml; sudo install -m 644 /tmp/webcodex-runner.service /etc/systemd/system/webcodex-runner.service; rm -f /tmp/webcodex-runner.service; sudo systemctl daemon-reload; sudo systemctl enable --now webcodex-runner.service" | Out-Null
